...
- **[web]**: Added `app/(auth)/signin/page.tsx` (UC-06), the sign-in screen from the `01a`–`01d` wireframes: `ParallaxScene` full-bleed behind a centred `PixelPanel` ("Fisher Timer" title, subtitle, a `PixelButton` wired to `signInWithGoogle()`, no logo — that composition is dropped until the sign art exists). No hanging-sign art yet; swapping it in later only touches `pixel.css`, per DESIGN_SYSTEM.md's "Replacing CSS surfaces with images".
  - `app/(auth)/signin/SignInError.tsx`: reads `?error=<code>` and renders the matching `PixelAlert` (01b/01c/01d), each state directly linkable for testing (`/signin?error=consent_denied`, `?error=code_exchange_failed`, `?error=account_creation_failed`). Isolated into its own component so only it needs the `<Suspense>` boundary `useSearchParams()` requires.
  - `lib/auth-error-messages.ts`: the three error messages (matching the wireframe copy) as their own module, shared by `lib/auth.ts`, `lib/mocks/auth.mock.ts` and `SignInError.tsx` without a value-level import cycle between `auth.ts` and its mock.
  - `lib/scenes/parallax-lake.ts`: the 9-layer lake preset (farthest/slowest to nearest/fastest), shared by `/signin` and the styleguide's "Scene" demo.
- **[web]**: Added `app/(auth)/welcome/page.tsx` (UC-06 S-1), the first-time setup screen from the `02a`–`02c` wireframes, reusing `/signin`'s exact background/header/panel shell rather than re-laying it out:
  - `components/SceneShell.tsx`: pulled the `ParallaxScene` + `Header` + centred content column out of the sign-in page into a shared layout component.
  - `PixelInput` wired to `completeFirstTimeSetup()` from `lib/auth.ts`. Both client-side states call the same `validateDisplayName()` from `lib/validate-display-name.ts` — no inline copy of its rule or messages: "Display name can't be empty." only shows once the user tries to continue (02b); "Display name is too long (max 30 characters)." shows live as they type past the limit (02c), since the same validator runs on every keystroke regardless of whether they've submitted yet. Corrected that shared function's wording (was "Name can't be...") to match the wireframes exactly — it's also what `lib/mocks/auth.mock.ts` and the styleguide demo derive their copy from, so both picked up the fix automatically.
  - Fixed `lib/mocks/auth.mock.ts`'s `MOCK_FIRST_TIME_SESSION`: its display name was `''`, but UC-06 S-1 prefills it from the Google profile at account creation (editable, not blank) — matches 02a's example value now.
- **[web]**: Wired the actual `/signin` → `/welcome` navigation and moved both under `app/(auth)/` (a Next.js route group — URLs stay `/signin` and `/welcome`) with a shared `app/(auth)/layout.tsx` rendering `SceneShell` once:
  - Since `app/(auth)/layout.tsx` persists across navigation between routes in the group, the background scene and header no longer remount when moving from `/signin` to `/welcome` — only the panel (each page's own content) swaps.
  - `/signin`'s button now calls `signInWithGoogle()` then `handleAuthCallback()` and routes on the result: `router.push(session.isFirstLogin ? "/welcome" : "/")`. A failure replaces the URL with `?error=<code>` instead of holding separate error state, so the existing `SignInError` component (which already reads that param) renders it — no second error-display path. In real mode `signInWithGoogle()`'s redirect means none of this runs in-page; it's what makes the flow testable end-to-end in mock mode ahead of a real callback route.
  - `?mockScenario=` on `/signin` now drives the whole click-to-landing flow in one step (e.g. `/signin?mockScenario=setup-default` + click → `/welcome`), reusing `handleAuthCallback()`'s existing scenario handling — no new mock plumbing.
  - `app/pixel.css`: `.pixel-panel--enter`, a brief rise-and-fade on mount (respects `prefers-reduced-motion` via the existing global `!important` override), applied to both panels so the swap between them reads as a transition rather than a jump cut. Deliberately did not force both panels to a shared fixed height (`/welcome`'s form content is naturally taller than `/signin`'s button) — that would leave visible dead space on the shorter panel; only the width (`max-w-sm`) is shared.
